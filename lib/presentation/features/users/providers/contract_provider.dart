...
class ContractProvider with ChangeNotifier {
  ...
  /// Active Contracts
  List<AllContractModel> _activeContracts = [];
  List<AllContractModel> get activeContracts => _activeContracts;

  Future<void> fetchActiveContracts() async {
    await _fetchContractsByStatus('active', (contracts) {
  ...
  Future<void> fetchCompletedContracts() async {
    await _fetchContractsByStatus('completed', (contracts) {
      _completedContracts = contracts;
    });
  }

  /// Paused Contracts
  List<AllContractModel> _pausedContracts = [];
  List<AllContractModel> get pausedContracts => _pausedContracts;

  Future<void> fetchPausedContracts() async {
    await _fetchContractsByStatus('paused', (contracts) {
      _pausedContracts = contracts;
    });
  }

  /// Cancelled Contracts
  List<AllContractModel> _cancelledContracts = [];
  List<AllContractModel> get cancelledContracts => _cancelledContracts;

  Future<void> fetchCancelledContracts() async {
    await _fetchContractsByStatus('cancelled', (contracts) {
      _cancelledContracts = contracts;
    });
  }

  /// Generic method to fetch contracts by status
  Future<void> _fetchContractsByStatus(
      String status,
      Function(List<AllContractModel>) onSuccess,
      ) async {
    _isLoading = true;
    _errorMessage = null;
    notifyListeners();
    ...
    try {
      final contracts = await repository.getContractsByStatus(status);
      onSuccess(contracts);
      _errorMessage = null;
    } catch (e) {
  ...
  Future<void> activateContract(String contractId) async {
    ...
    try {
      ...
      _showToast('Contract activated successfully');
    } catch (e) {
      _errorMessage = e.toString();
      _showToast(_errorMessage!);
    }
  ...
  Future<void> createContract(CreateContractRequest request) async {
    ...
    try {
      ...
      _showToast(_createContractResponse!.message);
    } catch (e) {
      _errorMessage = e.toString();
      _showToast(_errorMessage!);
    }
  ...
  Future<void> deleteContract(BuildContext context, String contractId) async {
    ...
    try {
      ...
      _showToast(_deleteResponse!.message);
    } catch (e) {
      _errorMessage = e.toString();
      _showToast(_errorMessage!);
    }
  ...
  Future<void> updatePaymentStatus(
    ...
    try {
      _paymentStatusResponse = await repository.updatePaymentStatus(
        contractId,
        request,
      );
    } catch (e) {
  ...
  /// Show toast without gravity to avoid errors
