...
class _JobDetailsPageState extends State<JobDetailsPage> {
  ...
  Future<void> _handleSubmit(BuildContext context) async {
    ...
    final double? estimatedCost = double.tryParse(_estimatedCostController.text.trim());

    if (estimatedCost == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter a valid estimated service cost.'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    if (_completionTimeController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please specify the estimated completion time.'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    await provider.submitApplication(
      JobApplicationSubmitRequest(
        jobId: widget.jobId ?? '',
        coverLetter: _coverLetterController.text.trim(),
        estimatedServiceCost: estimatedCost,
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: MainButton(
                            onTap: provider.isLoading ? null : () => Navigator.pop(context),
                            btn_title: 'Cancel',
                            btn_radius: 6,
                            buttonType: MainButtonType.outlined,
                        ...
                        Expanded(
                          child: MainButton(
                            onTap: provider.isLoading ? null : () => _handleSubmit(context),
                            btn_title: provider.isLoading ? 'Submitting...' : 'Submit',
                            btn_radius: 6,
                            btn_color: JAppColors.main,
                            btn_border_color: const Color(0xff7030F1),
                            title_color: Colors.white,
